...
#pragma mark - TestLayer_iPhone

@implementation TestLayer_iPhone

+ (CCScene *) scene
{
	CCScene *scene = [CCScene node];
	TestLayer_iPhone *layer = [TestLayer_iPhone node];
...
- (void) sceneItems
{
    ...
    bgSecond.position = ccp(winSize.width/2, winSize.height+bgSecond.contentSize.height/2+1);
    ...
    help = [CCLabelTTF labelWithString:LGLocalizedString(@"squareHelp", nil)
                            ...
                            dimensions:CGSizeMake(winSize.width-20, (temp.contentSize.height*2)*1.1)
    ...
    previousText = [CCLabelTTF labelWithString:LGLocalizedString(@"back", nil) fontName:kFontComicSans fontSize:textFontSize];
    ...
    nextText = [CCLabelTTF labelWithString:LGLocalizedString(@"next", nil) fontName:kFontComicSans fontSize:textFontSize];
    ...
    vkontakteButton = [CCLabelTTF labelWithString:@"В" fontName:kFontArialBlack fontSize:socialButtonsFontSize-4]; ///// soundOffButton
    ...
    sure = [CCLabelTTF labelWithString:LGLocalizedString(@"sure?", nil) fontName:kFontComicSans fontSize:textFontSize];
    sure.position = ccp(winSize.width*0.5, previousText.position.y+sure.contentSize.height);
    sure.color = ccc3(50, 50, 50);
    sure.opacity = 0;
	[self addChild:sure z:1];
    
    yesButton = [CCLabelTTF labelWithString:LGLocalizedString(@"yesUnsel", nil) fontName:kFontComicSans fontSize:textFontSize];
    yesButton.position = ccp(winSize.width*0.37, previousText.position.y);
    yesButton.color = ccc3(50, 50, 50);
    yesButton.opacity = 0;
    yesButton.anchorPoint = ccp(0.5, 0.5);
	[self addChild:yesButton z:1];
    
    noButton = [CCLabelTTF labelWithString:LGLocalizedString(@"noUnsel", nil) fontName:kFontComicSans fontSize:textFontSize];
    noButton.position = ccp(winSize.width*0.63, yesButton.position.y);
    noButton.color = ccc3(50, 50, 50);
    noButton.opacity = 0;
    noButton.anchorPoint = ccp(0.5, 0.5);
	[self addChild:noButton z:1];
	...
	stripe[2].position = ccp(winSize.width/2-winSize.width, winSize.height*0.65);
	[self addChild:stripe[2] z:1];
    
    stripe[3] = [CCSprite spriteWithTexture:stripeT];
	stripe[3].position = ccp(winSize.width/2+winSize.width, winSize.height*0.45);
	[self addChild:stripe[3] z:1];
    
    stripe[4] = [CCSprite spriteWithTexture:stripeT];
	stripe[4].position = ccp(winSize.width/2-winSize.width, winSize.height*0.25);
	[self addChild:stripe[4] z:1];
    
    stripe[5] = [CCSprite spriteWithTexture:stripeT];
	stripe[5].position = ccp(winSize.width/2-winSize.width, winSize.height*0.75);
	[self addChild:stripe[5] z:1];
    
    stripe[6] = [CCSprite spriteWithTexture:stripeT];
	stripe[6].position = ccp(winSize.width/2+winSize.width, winSize.height*0.55);
	[self addChild:stripe[6] z:1];
    
    stripe[7] = [CCSprite spriteWithTexture:stripeT];
	stripe[7].position = ccp(winSize.width/2-winSize.width, winSize.height*0.35);
	[self addChild:stripe[7] z:1];
    
    for (int i=0; i<=11; i++)
    {
        place[i] = ccp(stripe[1].position.x-stripe[1].contentSize.width/2+6+square[i].contentSize.width/2+square[i].contentSize.width*i+i, stripe[1].position.y);
    }
    for (int i=12; i<=23; i++)
    {
        place[i] = ccp(stripe[2].position.x-stripe[2].contentSize.width/2+6+square[i].contentSize.width/2+square[i].contentSize.width*(i-12)+(i-12), stripe[2].position.y);
    }
    for (int i=24; i<=35; i++)
    {
        place[i] = ccp(stripe[3].position.x-stripe[3].contentSize.width/2+6+square[i].contentSize.width/2+square[i].contentSize.width*(i-24)+(i-24), stripe[3].position.y);
    }
    for (int i=36; i<=47; i++)
    {
        place[i] = ccp(stripe[4].position.x-stripe[4].contentSize.width/2+6+square[i].contentSize.width/2+square[i].contentSize.width*(i-36)+(i-36), stripe[4].position.y);
    }
    for (int i=48; i<=59; i++)
    {
        place[i] = ccp(stripe[5].position.x-stripe[5].contentSize.width/2+6+square[i].contentSize.width/2+square[i].contentSize.width*(i-48)+(i-48), stripe[5].position.y);
    }
    for (int i=60; i<=71; i++)
    {
        place[i] = ccp(stripe[6].position.x-stripe[6].contentSize.width/2+6+square[i].contentSize.width/2+square[i].contentSize.width*(i-60)+(i-60), stripe[6].position.y);
    }
    for (int i=72; i<=83; i++)
    {
        place[i] = ccp(stripe[7].position.x-stripe[7].contentSize.width/2+6+square[i].contentSize.width/2+square[i].contentSize.width*(i-72)+(i-72), stripe[7].position.y);
    }
                          ...
                          color:(ccColor3B)color
{
    ...
    {
        buttonText.position = ccp(buttonBg.contentSize.width*0.9, position.y);
        buttonText.anchorPoint = ccp(0, 0.5);
    }
    else if (buttonText == nextText)
    {
        buttonText.position = ccp(winSize.width-buttonBg.contentSize.width*0.9, position.y);
...
- (void) test1SpritesAppear ////////// появление 1 теста
{
    ...
    [nextText setString:LGLocalizedString(@"next", nil)];
    if (mistakesOnScreen == 0) [previousText setString:LGLocalizedString(@"menu", nil)];
    if (mistakesOnScreen == 1) [previousText setString:LGLocalizedString(@"results", nil)];
    
    [kLGKit spriteFade:previousText duration:duration opacity:opacity];
...
- (void) test2SpritesAppear ////////// появление 2 теста
{
    ...
    [previousText setString:LGLocalizedString(@"back", nil)];
    if (mistakesOnScreen == 0) [nextText setString:LGLocalizedString(@"finish", nil)];
    if (mistakesOnScreen == 1) [nextText setString:LGLocalizedString(@"results", nil)];
...
- (void) finishSpritesAppear ////////// появление результатов
{
    ...
    [kLGKit spriteFade:tryAgainButtonImg duration:duration opacity:opacity];
    [kLGKit spriteFade:previousText duration:duration opacity:opacity];
...
- (void) buttonsDisappear
{
    ...
    if (finishTap == 0)
    {
        if ([kStandartUserDefaults integerForKey:@"goToMenuFromTest"] == 0) [kLGKit spriteFade:previousButtonBg duration:duration opacity:opacity];
        if ([kStandartUserDefaults integerForKey:@"goToMenuFromTest"] == 0) [kLGKit spriteFade:previousButtonStroke duration:duration opacity:opacity];
        [kLGKit spriteFade:previousButtonText duration:duration opacity:opacity];
        if ([kStandartUserDefaults integerForKey:@"goToMenuFromTest"] == 0) [kLGKit spriteFade:nextButtonBg duration:duration opacity:opacity];
        if ([kStandartUserDefaults integerForKey:@"goToMenuFromTest"] == 0) [kLGKit spriteFade:nextButtonStroke duration:duration opacity:opacity];
        [kLGKit spriteFade:nextButtonText duration:duration opacity:opacity];
    }
    if (finishTap == 1)
    {
...
- (void) test1SpritesDisappear ////////// исчезновение 1 теста
{
    for (int i=1; i<=3; i=i+2)
    {
    ...
    if (menuTap == 1  && gameFinish == 0)
    {
        [kLGKit spriteFade:sure duration:duration opacity:opacity];
        [kLGKit spriteFade:yesButton duration:duration opacity:opacity];
        [kLGKit spriteFade:noButton duration:duration opacity:opacity];
    }
...
- (void) test2SpritesDisappear ////////// исчезновение 2 теста
{
    for (int i=5; i<=7; i=i+2)
    {
        id a1 = [CCMoveBy actionWithDuration:0.04 position:ccp(winSize.width*0.05, 0)];
...
- (void) goToTryAgain
{
    [[CCDirector sharedDirector] replaceScene:[TestLayer_iPhone scene]];
}
...
- (void) ccTouchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (CGRectContainsPoint(nextButtonBg.boundingBox, touchPoint) && finishTap == 0) ////////// next
    {
        [kLGKit buttonSelect:nextButtonBg color:kColorLight];
        [self soundButton];
        z = 1;
    }
    else if (CGRectContainsPoint(previousButtonBg.boundingBox, touchPoint) && menuTap == 0) ////////// previous
    {
    ...
    else if (CGRectContainsPoint(yesButton.boundingBox, touchPoint) && gameFinish == 0 && (finishTap == 1 || menuTap == 1)) ////////// YES
    {
        [yesButton setString:LGLocalizedString(@"yesSel", nil)];
        [self soundChoose];
        z = 5;
    }
    else if (CGRectContainsPoint(noButton.boundingBox, touchPoint) && gameFinish == 0 && (finishTap == 1 || menuTap == 1)) ////////// NO
    {
        [noButton setString:LGLocalizedString(@"noSel", nil)];
        [self soundChoose];
        z = 6;
    }
...
- (void) ccTouchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (take == 1)
    {
        ...
        if (test == 1)
        {
            ...
            if (square[number].position.x >= place[10].x)
            {
        ...
        {
            if (square[number].position.x <= place[49].x)
            {
                square[number].position = ccp(place[49].x, square[number].position.y);
            }
            if (square[number].position.x >= place[58].x)
            {
                square[number].position = ccp(place[58].x, square[number].position.y);
            }
        }
...
- (void) ccTouchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (CGRectContainsPoint(nextButtonBg.boundingBox, touchPoint) && z == 1) ////////// next
    {
        ...
        if (test == 1 && ((gameFinish == 0 && mistakesOnScreen == 0) || (gameFinish == 1 && mistakesOnScreen == 1))) // next
        {
            [self test1SpritesDisappear];
            [self performSelector:@selector(test2SpritesAppear) withObject:nil afterDelay:0.2];
            [kLGKit performSelector:@selector(touchEnableWithTarget:) withObject:self afterDelay:0.5];
        }
        else if (test == 2 && gameFinish == 0 && mistakesOnScreen == 0) //  finish
        {
            [yesButton setString:LGLocalizedString(@"yesUnsel", nil)];
            [noButton setString:LGLocalizedString(@"noUnsel", nil)];
            
            id a2 = [CCDelayTime actionWithDuration:.1];
            id a3 = [CCFadeIn actionWithDuration:.1];
            [sure runAction:[CCSequence actions:a2, a3, nil]];
            
            id a4 = [CCDelayTime actionWithDuration:.1];
            id a5 = [CCFadeIn actionWithDuration:.1];
            [yesButton runAction:[CCSequence actions:a4, a5, nil]];
            
            id a6 = [CCDelayTime actionWithDuration:.1];
            id a7 = [CCFadeIn actionWithDuration:.1];
            [noButton runAction:[CCSequence actions:a6, a7, nil]];
            
            finishTap = 1;
            
            [kLGKit performSelector:@selector(touchEnableWithTarget:) withObject:self afterDelay:0.2];
        }
        else if (gameFinish == 1 && mistakesOnScreen == 0) // show mistakes
        {
            ...
            [self performSelector:@selector(test1SpritesAppear) withObject:nil afterDelay:0.3];
        ...
        else if (gameFinish == 1 && mistakesOnScreen == 1) // results
        {
            ...
            [self test2SpritesDisappear];
            
            [self performSelector:@selector(finishSpritesAppear) withObject:nil afterDelay:0.6];
        }
    }
    ...
    if (CGRectContainsPoint(previousButtonBg.boundingBox, touchPoint) && z == 2) ////////// previous
    {
        ...
        if (test == 2 && ((gameFinish == 0 && mistakesOnScreen == 0) || (gameFinish == 1 && mistakesOnScreen == 1))) // previous
        {
            [self test2SpritesDisappear];
            [self performSelector:@selector(test1SpritesAppear) withObject:nil afterDelay:0.2];
            [kLGKit performSelector:@selector(touchEnableWithTarget:) withObject:self afterDelay:0.5];
        }
        else if (test == 1 && gameFinish == 0 && mistakesOnScreen == 0) // menu
        {
            [yesButton setString:LGLocalizedString(@"yesUnsel", nil)];
            [noButton setString:LGLocalizedString(@"noUnsel", nil)];
    ...
    if (CGRectContainsPoint(yesButton.boundingBox, touchPoint) && z == 5) ////////// YES
    {
        self.isTouchEnabled = NO;
        ...
        id a1 = [CCFadeOut actionWithDuration:.3];
        [sure runAction:a1];
        
        id a2 = [CCFadeOut actionWithDuration:.3];
        [yesButton runAction:a2];
        
        id a3 = [CCFadeOut actionWithDuration:.3];
        [noButton runAction:a3];
    }
    else if (z == 5) [yesButton setString:LGLocalizedString(@"yesUnsel", nil)];
    
    if (CGRectContainsPoint(noButton.boundingBox, touchPoint) && z == 6) ////////// NO
    {
        self.isTouchEnabled = NO;
        ...
        id a3 = [CCFadeOut actionWithDuration:.1];
        [noButton runAction:a3];
    }
    else if (z == 6) [noButton setString:LGLocalizedString(@"noUnsel", nil)];
